  ...
  연하곤란: [
    {
      ...
      reason:
        '먹는 양을 줄이는 대신 음식의 형태를 바꾸는 것이 원칙입니다. 국제 표준(IDDSI)의 기준으로는 ' +
        '어른은 한 조각 1.5 cm 이하(포크 폭), 잘게 다진 단계는 4 mm 이하(포크 살 사이 간격)이고, ' +
        '포크로 눌러 엄지손톱이 하얘질 정도의 힘에 뭉개지면 부드러운 것입니다. ' +
        '촉촉하게 만들 때는 걸쭉한 소스를 쓰고, 국물처럼 묽은 것이 따로 흐르지 않도록 남는 물기는 따라 내십시오.',
    ...
    },
    {
      ...
      reason:
        ...
        '마른 빵·토스트·샌드위치는 국제 표준에서 아예 빼도록 되어 있고, 과일의 질긴 속껍질과 씨·껍질도 마찬가지입니다. ' +
        '한 입에 여러 질감이 섞인 음식(국물에 밥을 만 것 등)도 삼키기 어렵습니다.',
      evidence: 'G', refIds: ['iddsi']
    },
    {
      id: 'cond-dys-thin', level: 'caution', match: { foodIds: ['water', 'barley-tea', 'orange-juice'] },
      title: '물처럼 묽은 액체는 너무 빨리 흘러 조절하기 어렵습니다',
      reason:
        '묽은 액체는 흐르는 속도가 빨라 안전하게 조절하기 어려운 경우가 있습니다. 걸쭉하게 하면 흐름이 느려집니다. ' +
        '다만 어느 정도로 걸쭉하게 할지는 사람마다 달라, 연하 평가를 받고 담당 의료진이 정해 주는 단계를 따르셔야 합니다. ' +
        '임의로 되게 만들면 오히려 목에 남고 물을 적게 드시게 됩니다.',
      evidence: 'G', refIds: ['iddsi']
    }
  ],
  ...
  구강점막염: [
    {
      ...
      reason:
        ...
        '다만 근거의 무게는 정확히 알고 계시는 편이 낫습니다 — 국제 지침(MASCC/ISOO)은 ' +
        '"통증 조절과 식이 지원이 관리의 핵심"이라고 적으면서도, 어떤 음식을 빼야 하는지에 대해서는 ' +
        '근거가 불충분하고 서로 엇갈려 권고를 내지 못했습니다. ' +
        '위 목록은 통증이 생기는 기전과 진료 경험에 따른 조정이지 임상시험으로 증명된 목록이 아닙니다.',
      evidence: 'C', refIds: ['mascc-mucositis']
    },
    {
      ...
      reason:
        ...
        '여기에 더해 "구강 냉각"은 국제 지침이 실제로 권고하는 방법인데, 조건이 정해져 있습니다 — ' +
        '5-FU 를 짧게 밀어 넣는(볼루스) 방식으로 맞는 동안 30분간 얼음을 물고 있는 것, ' +
        '그리고 고용량 멜팔란으로 전처치하는 자가조혈모세포이식을 받는 경우입니다. ' +
        '이미 생긴 점막염을 낫게 하는 것이 아니라 생기는 것을 줄이는 예방법이고, ' +
        '지속 주입이나 먹는 항암제에는 해당되지 않습니다. 담당 의료진께 내 항암제가 여기에 해당하는지 확인하십시오.',
  ...
  체중증가: [
    {
      ...
      reason:
        ...
        '다만 치료를 받는 동안 살을 빼거나 체중 증가를 막는 개입이 도움이 되는지는 아직 근거가 부족해, ' +
        '미국임상종양학회는 2022년 지침에서 권고를 내지 않았습니다 — 하지 말라는 뜻이 아니라 아직 모른다는 뜻입니다. ' +
        '그래서 치료 중에는 감량을 목표로 삼지 마시고, 열량을 줄이기보다 같은 열량에서 ' +
        '단백질과 채소의 비중을 올리는 편이 실천하기도 쉽고 안전합니다.',
  ...
  위절제후: [
    {
      ...
      reason:
        ...
        '위절제 후에 지켜야 할 "정해진 식단"이 따로 있는 것은 아니고, 증상을 보아 가며 맞추는 것이 원칙입니다.',
      evidence: 'C', refIds: ['gastrectomy-nutr']
    },
    {
      ...
      evidence: 'B', refIds: ['gastrectomy-nutr', 'b12-gastrectomy-meta']
    }
  ],
  ...
  장루보유: [
    {
      ...
      reason:
        ...
        '다만 이 조언의 무게는 알고 계시는 편이 낫습니다 — 장루 식이 문헌을 훑은 종설은 ' +
        '"조언이 흔히 주어지지만 서로 엇갈리고 불충분하며 질 높은 연구가 부족하다"고 정리했습니다. ' +
        '못 드시는 목록이 아니라 처음에 조심할 목록으로 보시고, 괜찮으면 늘려 가시면 됩니다.',
      /*
       * 인용을 두 번 갈아 끼웠다. ESPEN 두 판에 '섬유' 가 0건이라 뗐고,
       * 그 자리에 넣은 ERAS 대장수술 지침에도 섬유·장루 식이 이야기는 없다 —
       * 그 지침이 다루는 것은 수술 전후 관리 항목이지 장루 식단이 아니다.
       * 대신 장루 식이 문헌 전체를 훑어 '근거가 얇다' 는 것을 정리한 종설을 찾아 붙였다 —
       * 조언과 그 한계를 같은 출처로 말할 수 있게 됐다.
       */
      evidence: 'C', refIds: ['ileostomy-diet-review']
    },
    {
  ...
  신기능저하: [
    {
      ...
      reason:
        ...
        '건강한 성인의 칼륨 충분섭취량은 하루 3,500 mg 이지만, 신기능이 떨어진 분께는 이 값이 그대로 적용되지 않습니다 — ' +
        '얼마까지 드실지는 검사 수치를 보고 담당 의료진이 정합니다. ' +
    ...
    },
    {
      ...
      reason:
        ...
        '콜라·가공치즈·햄류가 대표적입니다. ' +
        '건강한 성인의 인 권장섭취량은 하루 700 mg, 상한은 3,500 mg 입니다 — ' +
        '같은 700 mg 이라도 첨가물로 들어온 쪽이 몸에 더 많이 남는다는 것이 요점입니다.',
      evidence: 'G', refIds: ['kdri2020']
    }
  ],
  ...
  고혈압: [
    {
      ...
      /*
       * '2,000 mg 이하' 는 2015 판의 목표섭취량이다. 2020 판의 기준은 2,300 mg 이고 뜻도 다르다 —
       * "이 아래로 내려가라" 가 아니라 "이보다 높게 드시면 줄이는 만큼 위험이 낮아진다" 이다.
       */
      title: '나트륨은 지금보다 줄이는 것이 목표입니다 — 기준은 하루 2,300 mg',
      reason:
        '2020 한국인 영양소 섭취기준은 나트륨 만성질환위험감소섭취량을 하루 2,300 mg 으로 정했습니다. ' +
        '65~74세는 2,100 mg, 75세 이상은 1,700 mg 으로 더 낮습니다. ' +
        '이 값은 그 아래로 반드시 내려가라는 뜻이 아니라, 그보다 많이 드시고 계시면 줄이는 만큼 ' +
        '만성질환 위험이 낮아진다는 뜻입니다. 한국인 평균이 3,255 mg 이니 대부분 줄일 여지가 있습니다. ' +
