  ...
  '오심·구토': [
    {
      ...
      /*
       * 용량이 빠져 있었다. "생강이 도움이 된다" 만 알려 드리면 얼마나 드실지는 짐작에 맡기게 된다.
       * 원 시험은 하루 0.5~1.0 g 에서 효과가 가장 컸고 1.5 g 이 더 낫지 않았다 —
       * 많이 드실수록 좋다는 뜻이 아니므로 그 숫자를 함께 적는다.
       */
      reason:
  ...
  체중증가: [
    {
      ...
      /*
       * '치료 중 늘어난' 이라고 단정해 두었더니 치료를 마치신 분께도 그대로 떴다.
       * 체중이 느는 일은 치료를 마친 뒤에도 흔하다 — 항호르몬 치료와 줄어든 활동량 탓이다.
       * 어느 쪽이든 같은 조언이 맞으므로, 한쪽으로 단정하지 않고 둘 다 적는다.
       */
      title: '이때 늘어난 체중은 나중에 잘 빠지지 않습니다',
      reason:
        '암 치료를 받는 동안에도, 마친 뒤에도 체중이 느는 일은 흔합니다. ' +
        '치료 중에는 오심을 달래려 탄수화물 위주로 먹게 되고 스테로이드가 식욕을 올리며, ' +
        '치료를 마친 뒤에는 항호르몬 치료와 줄어든 활동량이 겹칩니다. ' +
        '문제는 이때 늘어난 것이 근육이 아니라 지방이고, 한번 붙으면 잘 빠지지 않는다는 점입니다. ' +
    ...
    },
    {
      ...
      reason:
        ...
        '먹는 양을 참는 방식은 어느 시기에나 오래 가지 못합니다 — 치료 중에는 더 그렇습니다.',
    ...
    },
    {
      ...
      /*
       * '이 시기에 무리한 감량을 하면' 이라고 적어 두었는데, 그 '이 시기' 가
       * 치료 중을 가리키고 있었다. 치료를 마치신 분께는 오히려 체중 관리가
       * 권고 사항이라 정반대로 읽힌다. 어느 시기에나 맞는 말로 바꾼다 —
       * 빠르게 빼면 근육부터 빠진다는 것은 치료 후에도 그대로다.
       */
      reason:
        '체중계 숫자가 그대로여도 근육이 줄고 지방이 느는 경우가 많습니다. ' +
        '단백질을 유지하면서 저항운동을 더해야 몸의 구성이 나빠지지 않습니다. ' +
        '치료 중이라면 감량 자체가 목표가 아니고, 치료를 마치셨더라도 ' +
        '한 달에 체중의 2 % 를 넘겨 빼면 근육부터 빠집니다.',
