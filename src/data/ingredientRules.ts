...
export const INGREDIENT_RULES: IngredientRule[] = [
  ...
  /* ── 간 관련 ─────────────────────────────────────────── */
  {
    name: '가르시니아캄보지아',
    match: ['가르시니아'],
    /* 간독성 때문에 어느 시기에도 권하지 않지만, 이유는 시기에 따라 다르게 적는다 */
    base: 'avoid',
    reason:
      '체지방 감소를 표방하는 원료인데, 간독성 사례가 반복 보고되어 식약처도 주의를 안내한 바 있습니다. ' +
      '체중 관리가 필요하시더라도 식사와 운동으로 하시는 편이 낫습니다.',
    evidence: 'C',
    refIds: ['espen2021', 'easl-nutrition'],
    duringTreatment: {
      level: 'avoid',
      reason:
        '간독성 사례가 반복 보고된 원료입니다. 게다가 치료 중 체중 감량은 목표가 아니며, ' +
        '이 시기에 빠지는 것은 지방이 아니라 근육입니다.'
    }
  },
  {
    name: '녹차추출물(카테킨·EGCG)',
  ...
  },
  {
     ...
     * 시기에 따라 말이 뒤집히는 원료다.
     *
     * '치료 중 체중 감량은 목표가 아닙니다' 라고만 적어 두었더니,
     * 치료를 마치고 체중 관리가 오히려 권고 사항이 된 분께도 그대로 떴다.
     * 그분께 "체중 감량은 목표가 아니다" 라고 말하는 것은 앱이 다른 곳에서
     * 하는 이야기와 정면으로 어긋난다.
     *
     * 다만 치료를 마쳤다고 이 원료를 권할 수는 없다. 체중은 식사와 운동으로
     * 관리하는 것이고, 이 원료들은 효과가 미미하거나 간독성이 보고되어 있다.
     * 그래서 판정은 '주의' 로 낮추되 이유를 바꿔 적는다.
     */
    name: '체지방 감소 표방 원료',
    match: ['체지방', '다이어트', '슬림', '가르시니아', '카르니틴', '녹차카테킨'],
    base: 'caution',
    reason:
      '체중 관리가 필요하시더라도 이런 원료로 하실 일은 아닙니다. 효과는 대개 미미하고, ' +
      '가르시니아·녹차추출물처럼 간독성이 보고된 것도 있습니다. ' +
      '치료를 마친 뒤의 체중 관리는 식사와 운동으로 하는 것이 근거가 분명합니다.',
    evidence: 'G',
    refIds: ['espen2021', 'asco2022'],
    duringTreatment: {
      level: 'avoid',
      reason:
        '치료 중에는 체중 감량 자체가 목표가 아닙니다. 이 시기에 빠지는 것은 지방이 아니라 근육이고, ' +
