import type { CancerProfile } from '../types'

export const colorectal: CancerProfile = {
  id: 'colorectal',
  name: '대장암',
  summary:
    '대장암은 식이 근거가 가장 두껍게 쌓인 암입니다. 가공육과 적색육은 위험을 높이고, 식이섬유·통곡물·유제품은 낮추는 쪽으로 ' +
    'WCRF 가 "convincing/probable" 등급을 부여했습니다. 진단 후 식이섬유 섭취가 많은 환자에서 사망률이 낮았다는 코호트 결과도 있습니다. ' +
    '다만 수술 직후나 장 협착·장루 보유 상태에서는 오히려 섬유를 줄여야 하므로, 시기 구분이 중요합니다.',
  keyIssues: [
    '가공육·적색육 섭취',
    '수술 직후·장 협착 시의 저잔사식과, 회복 후의 고섬유식이 정반대 방향이라는 점',
    '장루 보유 시 가스·냄새·수분 손실 관리',
    '골반 방사선치료로 인한 설사',
    '비만과 신체활동 부족'
  ],
  target: {
    kcalPerKg: [25, 30],
    proteinPerKg: [1.0, 1.5],
    naLimit: 2000,
    fiberTarget: [25, 35],
    fluidPerKg: 30,
    notes: [
      '섬유 목표는 회복기 이후 기준입니다. 수술 직후·협착·급성 설사 시에는 하루 10 g 이하의 저잔사식으로 낮춥니다.'
    ]
  },
  rules: [
    {
      id: 'crc-processed-meat',
      level: 'avoid',
      match: { tags: ['가공육'] },
      title: '가공육은 대장암에서 가장 명확한 회피 대상입니다',
      reason:
        'IARC 는 가공육을 Group 1 로 분류하면서, 10편의 연구를 모아 하루 50 g 섭취마다 대장암 위험이 ' +
        '약 18 % 증가한다고 정리했습니다. 햄 2~3장, 소시지 1개 정도가 그 50 g 에 해당합니다. ' +
        '부대찌개처럼 가공육이 주재료인 메뉴가 우선 조정 대상입니다.',
      evidence: 'A',
      refIds: ['iarc114', 'wcrf2018']
    },
    {
      id: 'crc-red-meat',
      level: 'caution',
      match: { tags: ['적색육'] },
      title: '적색육은 주당 조리 후 500 g 이내로 제한하세요',
      reason:
        'WCRF/AICR 는 적색육(소·돼지·양)을 주당 조리 후 350~500 g 이내로 권고합니다. 하루 한 끼 고기반찬 정도의 양입니다. ' +
        '적색육은 가공육과 등급이 다릅니다 — IARC 분류는 Group 2A(아마도 발암성)이고 근거도 "제한적"이라고 적혀 있습니다. ' +
        '인과관계가 맞다고 가정했을 때의 추정치가 하루 100 g 당 대장암 위험 17 % 증가입니다. ' +
        '헴철이 장내에서 N-니트로소 화합물 생성을 촉진하는 것이 주요 기전으로 지목됩니다. ' +
        '조리 후 500 g 은 생고기로 700~750 g 쯤 되니, 장 보실 때 이 무게로 가늠하시면 됩니다. ' +
        '완전히 끊을 필요는 없으며, 가금류·생선·콩으로 일부를 대체하는 방식이 현실적입니다.',
      evidence: 'A',
      refIds: ['wcrf2018', 'iarc114']
    },
    {
      id: 'crc-fiber',
      level: 'prefer',
      match: { tags: ['고식이섬유'] },
      phases: ['survivorship', 'all'],
      title: '회복 후에는 식이섬유를 하루 30 g 이상으로 올리세요',
      reason:
        'WCRF/AICR 는 식이섬유를 식품으로 하루 30 g 이상 드시도록 권고합니다. ' +
        '1~3기 대장암 환자 1,575명을 8년 추적한 전향적 코호트에서, 진단 후 섬유를 하루 5 g 더 드실 때마다 ' +
        '대장암 사망이 22 % 낮았습니다(위험비 0.78). 진단 전보다 섬유를 늘리신 분들에서는 5 g 마다 18 % 씩 낮아졌습니다. ' +
        '어디서 온 섬유인지가 갈립니다 — 곡물 섬유가 가장 뚜렷했고(5 g 마다 33 % 낮음), 채소 섬유는 전체 사망에서만, ' +
        '과일 섬유는 연관이 확인되지 않았습니다. 잡곡밥과 통곡물 쪽을 먼저 늘리시는 편이 낫습니다.',
      evidence: 'B',
      /* CALGB 89803(Van Blarigan) 은 생활 점수를 본 연구지 섬유의 용량-반응을 보고하지 않는다 */
      refIds: ['song2018fiber', 'wcrf2018']
    },
    {
      id: 'crc-lowresidue',
      level: 'prefer',
      match: { tags: ['저잔사', '부드러움'] },
      phases: ['post_op'],
      /*
       * '4~6주에 걸쳐' 라고 적어 두었으나 ERAS 지침의 방향은 그 반대다.
       * 원문은 "정상 식사 재개가 늦어지면 감염 합병증과 회복 지연이 늘어난다" 고 못 박고,
       * "대부분의 환자는 수술 당일부터 음식과 경구영양보충을 시작해야 한다"(강한 권고) 로 끝난다.
       * 저잔사식이 언급되는 자리는 '맑은 유동식보다 낫다' 는 비교이지 오래 제한하라는 뜻이 아니다.
       */
      title: '수술 직후에는 저잔사식으로 시작하되, 오래 끌지 않는 것이 중요합니다',
      reason:
        '문합부가 아무는 동안 대변량과 장운동 부담을 줄이려고 흰밥·흰죽·껍질 벗긴 부드러운 채소로 시작합니다. ' +
        '맑은 유동식보다 이런 저잔사식이 오심이 적고 장 기능이 빨리 돌아오며 재원 기간도 짧았다는 보고가 있습니다. ' +
        '다만 여기서 중요한 것은 "얼마나 오래 제한하느냐" 입니다 — 수술 후 회복 지침(ERAS)은 ' +
        '정상 식사 재개가 늦어질수록 감염 합병증이 늘고 회복이 느려진다는 점을 근거로, ' +
        '대부분의 환자는 수술 당일부터 음식과 영양보충 음료를 시작해야 한다고 강하게 권고합니다. ' +
        '저잔사식은 며칠 동안의 출발점이지 몇 주씩 이어 갈 식단이 아닙니다. ' +
        '언제 무엇을 올릴지는 회복 속도에 따라 다르니 담당 의료진의 지시를 따르십시오.',
      evidence: 'G',
      /*
       * ESPEN 암 영양 지침(2017 전체·2021 축약) 전문을 검색해도 이 내용은 없다 —
       * '섬유' 는 두 판 통틀어 0건, '소량 다회' 도 0건, 'B12' 도 0건이다.
       * 인용은 실제로 그 말을 하는 문헌에만 단다.
       */
      refIds: ['eras-colorectal']
    },
    {
      id: 'crc-highfiber-stricture',
      // 수술을 받았다고 모두 협착이나 장루가 있는 것은 아니다.
      // 장루보유를 동반 상태로 고르면 cond-stoma-fiber 가 함께 걸린다.
      level: 'caution',
      match: { tags: ['고식이섬유', '거친질감'] },
      phases: ['post_op'],
      title: '수술 직후 질긴 섬유는 조심하세요 — 협착·장루가 있다면 특히',
      reason:
        '버섯 밑동, 팽이버섯, 나물 줄기, 견과류처럼 질긴 섬유는 좁아진 부위나 장루 입구를 막을 수 있습니다. ' +
        '충분히 잘게 썰고 오래 씹는 것이 기본이며, 증상이 있으면 해당 식품을 잠시 제외합니다.',
      evidence: 'G',
      refIds: ['espen2021']
    },
    {
      id: 'crc-calcium-dairy',
      level: 'prefer',
      match: { tags: ['고칼슘'], groups: ['우유·유제품'] },
      title: '유제품과 칼슘은 대장암 위험을 낮추는 쪽입니다',
      reason:
        'WCRF/AICR 는 유제품과 칼슘 섭취가 대장암 위험을 낮춘다는 근거에 "probable" 등급을 부여했습니다. ' +
        '칼슘이 장내 담즙산과 유리지방산을 결합해 점막 자극을 줄이는 것이 기전으로 제시됩니다. ' +
        '단, 이 방향은 전립선암에서는 정반대이므로 암종을 구분해서 보아야 합니다.',
      evidence: 'B',
      refIds: ['wcrf2018']
    },
    {
      id: 'crc-vitd',
      level: 'info',
      match: { supplementCategories: ['비타민D'] },
      title: '비타민 D 는 결핍 교정 수준까지가 근거가 있는 범위입니다',
      reason:
        '전이성 대장암 환자 139명을 대상으로 고용량 비타민 D3 를 항암제에 추가한 SUNSHINE 2상 연구에서 ' +
        '무진행생존은 13.0개월 대 11.0개월로 통계적 차이가 없었고(다변량 분석에서만 신호), ' +
        '전체 생존은 양쪽 모두 24.3개월로 똑같았습니다. ' +
        '예방 쪽 근거는 더 분명합니다 — 건강한 성인 25,871명에게 비타민 D 2,000 IU 를 5.3년간 드리게 한 VITAL 연구에서 ' +
        '암 발생은 줄지 않았고(위험비 0.96), 대장암도 마찬가지였습니다(1.09). ' +
        '결핍이 있으면 교정하되, 항암 효과나 예방을 기대한 초고용량 복용을 뒷받침할 근거는 아직 없습니다.',
      evidence: 'A',
      refIds: ['ng2019', 'manson2019']
    },
    {
      id: 'crc-diarrhea-rt',
      level: 'prefer',
      match: { tags: ['저잔사', '저FODMAP'] },
      phases: ['during_rt'],
      title: '골반 방사선치료 중 설사가 있으면 잔사와 유당을 줄이세요',
      reason:
        '골반 조사 중 급성 장염으로 설사가 흔합니다. 이 시기에는 고섬유·고지방·유당·카페인·매운 음식이 증상을 악화시킵니다. ' +
        '흰죽·바나나·감자·닭가슴살처럼 잔사가 적고 부드러운 음식으로 옮기고, 수분과 전해질을 함께 보충합니다.',
      evidence: 'G',
      refIds: ['espen2021']
    },
    {
      id: 'crc-lifestyle',
      level: 'prefer',
      match: { groups: ['채소', '과일', '곡류·전분'] },
      phases: ['survivorship'],
      title: '가이드라인을 잘 지킨 환자군에서 실제로 생존이 좋았습니다',
      reason:
        'CALGB 89803 임상시험에 참여한 3기 대장암 환자 992명을 7년 추적했더니, 미국암학회의 식사·운동·체중 권고를 ' +
        '가장 잘 따른 군(6점 만점에 5~6점)은 가장 못 따른 군(0~1점)보다 사망 위험이 42 % 낮았습니다(위험비 0.58). ' +
        '여기에 절주까지 넣어 8점으로 매기면 차이가 더 벌어졌습니다(위험비 0.49). ' +
        '단일 식품이 아니라 전체 생활 패턴의 문제라는 것을 보여준 결과입니다.',
      evidence: 'B',
      refIds: ['vanblarigan2018', 'acs2022']
    }
  ],
  phaseNotes: {
    post_op:
      '저잔사식으로 시작해 4~6주에 걸쳐 섬유를 늘립니다. 장루가 있다면 가스를 만드는 콩류·양파·탄산음료를 처음에는 줄이고, ' +
      '수분·나트륨 손실이 크므로 물을 충분히 마십니다.',
    during_rt:
      '골반 조사 중에는 설사 대응이 중심입니다. 증상이 심하면 유당·고지방·고섬유를 한꺼번에 줄여 보고 하나씩 다시 넣습니다.',
    during_chemo:
      '옥살리플라틴 사용 중에는 찬 음식·찬 음료가 인후 경련과 손발 저림을 유발할 수 있어 미지근하게 드시는 편이 좋습니다.',
    survivorship:
      '섬유·통곡물·유제품을 늘리고, 가공육을 줄이며, 체중과 활동량을 관리합니다.'
  },
  refIds: ['wcrf2018', 'iarc114', 'song2018fiber', 'vanblarigan2018']
}
