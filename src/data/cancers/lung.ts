...
export const lung: CancerProfile = {
  ...
  rules: [
    ...
    },
    {
      ...
      reason:
        /*
         * '2~3주째' 를 MASCC/ISOO 점막염 지침에 달아 두었는데, 그 지침 전문에는
         * 시점에 관한 서술이 없다 — 내용은 전부 중재(구강관리·벤지다민·광선치료·글루타민·꿀)다.
         * 시점은 분할조사의 통상적 경과이므로 지침 인용 없이 임상 경과로 적고,
         * MASCC 는 그것이 실제로 말하는 것(통증 조절과 식이 지원이 관리의 핵심)에만 붙인다.
         */
        '흉부 방사선치료가 2주를 넘어서면 식도 점막염으로 삼킬 때 통증이 생기는 경우가 많습니다. ' +
        '뜨겁고 맵고 신 음식, 거친 음식이 통증을 키우므로 미지근한 죽·계란찜·부드러운 두부·영양음료 위주로 옮깁니다. ' +
        '통증 때문에 먹는 양이 줄어드는 것이 이 시기 체중 감소의 주된 원인이라, ' +
        '점막염 지침도 통증 조절과 식이 지원을 관리의 핵심 요소로 꼽습니다.',
      evidence: 'G',
