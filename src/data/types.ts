...
export type FoodTag =
  // 조리·가공 방식
  | '생식'            // 익히지 않음 — 호중구감소증 시 주의 (생채소·생과일 포함)
  /*
   * 익히지 않은 **동물성** 식품 — 회·초밥·육회·생굴.
   *
   * '생식' 과 나눈 이유가 있다. 생채소·생과일까지 막는 이른바 '호중구감소증 식단' 은
   * 근거가 없어서, 생식 규칙은 식품군으로 동물성만 걸러 내고 있었다.
   * 그런데 그 걸름망이 '회덮밥'(밥·면·죽 요리)·'물회'(반찬) 처럼
   * 요리 이름으로 분류된 것을 함께 걸러 버렸다.
   * 이 태그는 붙는 순간 이미 '날것 동물성' 이라는 뜻이므로 식품군을 따지지 않는다.
   */
  | '생식동물성'
  /*
   * 익히지 않은 새싹채소 — 숙주·알팔파·브로콜리 새싹·무순.
   *
   * '생채소는 씻어 드시면 됩니다' 의 예외다. 미국 FDA 는 생 새싹채소를
   * 위험 높은 쪽에 따로 올려 두고 낮은 쪽으로 '익힌 새싹' 을 든다 —
   * 씻는 것으로 해결되지 않기 때문이다(따뜻하고 습한 발아 조건 자체가 균에 유리하다).
   * 그래서 생채소 일반과 나누어 태그를 세운다.
   */
  | '생새싹'
  | '발효'
  | '훈제'
 ...
 * 고르실 수 있는 요리 계통.
 *
 * '무관' 은 어느 상에나 오르는 것(과일·우유 같은)이라 고르는 목록에는 넣지 않는다.
 * 한식은 늘 열려 있다 — 고르지 않으셔도 한식은 나온다. 여기서 고르시는 것은
 * '한식 말고 무엇을 더 보고 싶으신가' 에 가깝다.
 */
export const CUISINE_CHOICES: Cuisine[] = ['한식', '양식', '중식', '일식', '동남아']
