...
import { MEAL_SLOTS, SNACK_SLOTS, isSnack } from '../data/types'
...
const SLOT_BY_GROUP: Record<FoodGroup, MealSlot[]> = {
   ...
   * 우유·유제품은 어느 끼니에나 오른다.
   *
   * 예전에는 아침과 간식에만 두었다. 서양식 아침(빵·우유)을 그대로 옮겨 놓은 셈인데,
   * 그 바람에 간식이 차면 우유와 요거트가 갈 곳이 아침밖에 없었다.
   * '아침이 저녁보다 무거움' 을 여섯 번 파고들어도 원인을 못 짚다가,
   * 무거운 아침 298건에 무엇이 들어 있는지 세어 보고서야 드러났다 —
   * 요거트 과일볼 72 · 우유 58 · 락토프리 우유 46. 유제품이 위쪽을 차지하고 있었다.
   * 코드 주석은 이미 그렇게 적어 두고 있었다("우유는 아침 아니면 간식이다").
   *
   * 점심·저녁에 우유 한 잔을 드시는 것은 이상한 일이 아니고,
   * 후식은 끼니 맨 뒤로 줄을 세우므로 상차림도 흐트러지지 않는다.
   *   아침이 저녁보다 무거움 2.72% → 0.96%
   */
  '우유·유제품': ['아침', '점심', '저녁', ...SNACK_SLOTS],
  '유지·당류': ['점심', '저녁'],
  '국·탕·찌개': ['아침', '점심', '저녁'],
  '밥·면·죽 요리': ['아침', '점심', '저녁'],
  '반찬·조림·볶음': ['아침', '점심', '저녁'],
  가공식품: [...SNACK_SLOTS],
  음료: ['점심', '저녁', ...SNACK_SLOTS],
  '간식·디저트': [...SNACK_SLOTS],
  '외식·프랜차이즈': ['점심', '저녁'],
  /*
 ...
 * 반찬은 더 오래 미룬다.
 *
 * 사흘이면 다시 풀리는데, 점수가 가장 높은 것은 풀리자마자 도로 올라온다.
 * 그래서 반찬 160종을 갖춰 두고도 서른 종으로 돌았다 — 두부부침이 나흘에 한 번꼴이었다.
 * 엿새로 늘리면 한 주에 한 번 남짓이 되어 상이 훨씬 다양해진다.
 * 고를 것이 동나면 EMPTY_RECENT 로 되돌아가므로 굶기지는 않는다.
 */
const ANCHOR_BLOCK_DAYS = 6
/** 그 뒤로도 얼마 동안은 뒤로 미룬다 */
const REPEAT_FADE_DAYS = 7
/**
 * 이 열량을 넘는 것만 '되풀이' 로 친다. 곁들이는 것까지 막을 이유는 없다.
 *
 * 다만 반찬은 열량과 무관하게 되풀이로 친다.
 * 두부(부침용)은 114 kcal 라 이 뚜껑 아래로 빠져 나가, 300일 중 146일에 올랐다.
 * 나물·무침·데침 같은 반찬이 대개 100 kcal 아래여서 다 같은 처지였다 —
 * 상을 세우는 것들이 통째로 회전에서 빠져 있었던 셈이다.
 * 반찬은 열량이 적어도 반찬이다.
 */
const REPEAT_MIN_KCAL = 150

/** 되풀이로 볼 것인가 — 반찬이거나, 곁들임이라도 열량이 실한 것 */
function countsAsRepeat(f: Food, kcal: number): boolean {
  return isAnchorDish(f) || kcal >= REPEAT_MIN_KCAL
}

/** 이 음식은 며칠 동안 다시 권하지 않는가 */
function blockDays(f: Food): number {
  return isAnchorDish(f) ? ANCHOR_BLOCK_DAYS : REPEAT_BLOCK_DAYS
}
...
export function buildDayMenu(
  ...
  const meals: Record<MealSlot, MenuEntry[]> = { 아침: [], 오전간식: [], 점심: [], 오후간식: [], 저녁: [] }
  ...
  for (const { food, servings, meal } of keep) {
    ...
    /*
     * 같은 것을 같은 끼니에 두 번 담으셨으면 하나로 합친다.
     *
     * 저장소는 담으실 때 합쳐 주지만, 예전 기록이나 밖에서 들어온 목록은 그렇지 않다.
     * 그대로 두면 한 끼니에 두부부침이 두 줄로 서고, 화면에서는 같은 열쇠가 겹쳐
     * 지우기 단추가 엉뚱한 것을 지운다.
     */
    const twin = meals[slot].find((e) => e.food.id === food.id)
    if (twin) twin.servings = Math.round((twin.servings + servings) * 10) / 10
    else meals[slot].push({ food, servings, origin: 'chosen', seasonal: isSeasonal(food, season) })
  ...
  const quota: Record<MealSlot, number> = {
    ...
    저녁: midKcal * shares['저녁'],
    오전간식: midKcal * shares['오전간식'], 오후간식: midKcal * shares['오후간식']
  }

  /*
   * 3-0) 끼니마다 밥부터 앉힌다.
   *
   * 처음에는 다 채운 뒤에 빈 상을 찾아 밥을 끼워 넣었다. 그런데 밥 한 공기는 300 kcal 이고,
   * 그때쯤이면 남은 여유가 50~240 kcal 이라 들어갈 자리가 없다.
   * 곁들임 하나를 내려 자리를 만들어 보아도 모자랐다 — 스무 끼니 중 넷이 밥 없이 나갔다.
   *
   * 순서가 틀렸던 것이다. 상은 밥을 놓고 그 둘레를 채우는 것이지,
   * 다 채운 다음 밥을 비집어 넣는 것이 아니다. 그래서 여기서 먼저 앉힌다.
   * 뒤따르는 채우기가 남은 몫을 알아서 맞춘다.
   */
  {
    for (const slot of ['아침', '점심', '저녁'] as MealSlot[]) {
      const here = meals[slot]
      const roles = here.map((e) => mealRole(e.food))
      /* 한 그릇 음식을 고르셨으면 그것으로 족하다 — 국수에 밥을 얹지 않는다 */
      if (roles.includes('staple') || roles.includes('onedish')) continue

      const cur = running()
      const room = target.kcal[1] - (cur.kcal ?? 0)
      if (room <= 0) continue

      /*
       * 밥은 하루에 두 번 나와도 된다.
       *
       * 다른 음식은 같은 날 두 번 오르지 않게 막는다. 그런데 이분께 올릴 수 있는
       * 밥이 세 종뿐인 경우가 있어, 세 끼를 채우고 나면 바닥이 났다 —
       * 그래서 저녁상에 밥이 없었다.
       * 한국에서 아침에도 쌀밥, 저녁에도 쌀밥은 조금도 이상하지 않다.
       * 아직 안 쓴 것을 앞에 두되, 없으면 쓴 것을 다시 쓴다.
       */
      const rice = candidates
        /*
         * 죽과 국수도 주식이다.
         *
         * 밥만 앉혔더니, 삼키기 어려운 분께 죽이 스무 날 동안 한 번도 안 나왔다.
         * 밥이 먼저 자리를 차지해 버려서 뒤에 죽이 올라올 길이 없었던 것이다.
         * 그분들께는 죽이 밥보다 나은 주식이다 — 같은 자리를 두고 겨루게 둔다.
         */
      ...
      /* 엇비슷하면 날마다 다르게 — 늘 같은 밥이 오르지 않게 */
      /* 고를 때는 아직 안 쓴 것들 안에서만 섞는다 — 안 그러면 굳이 되풀이하게 된다 */
      const fresh = rice.filter((c) => !used.has(c.food.id))
      const pool = fresh.length > 0 ? fresh : rice
      const near = pool.filter((x) => (x.bonus - x.penalty) >= ((pool[0]?.bonus ?? 0) - (pool[0]?.penalty ?? 0)) - 10)
      const put = near[((dayIndex % Math.max(1, near.length)) + near.length) % Math.max(1, near.length)]
      if (!put) continue

      /*
       * 밥도 그분 몫에 맞춰 담는다.
       *
       * 한 공기를 그대로 세 끼에 놓았더니, 목표가 1,140 kcal 인 38 kg 환자분의
       * 하루 가운데 800 kcal 가 밥이 되었다. 남은 자리에 닭백숙 한 그릇이 들어가자
       * 하루가 1,341 로 넘어갔다.
       * 밥공기 크기는 사람마다 다르다 — 이 끼니 몫의 절반을 넘으면 반 공기로 담는다.
       */
      const part = put.kcal > quota[slot] * 0.55 ? 0.5 : 1
      if (foodContribution(put.food, part).kcal! > room) continue

      used.add(put.food.id)
      groupCount.set(put.food.group, (groupCount.get(put.food.group) ?? 0) + 1)
  ...
  for (let guard = 0; guard < 40; guard++) {
     ...
     */
    /*
     * 막혔을 때 이력을 통째로 버리지 않는다.
     *
     * 예전에는 곧바로 EMPTY_RECENT 로 떨어졌다. 그러면 엊그제 드신 것까지 다시 후보가 되고,
     * 점수가 가장 높은 그 반찬이 도로 1등으로 올라온다.
     * 회전 창을 엿새로 늘리고 회전 폭을 넓혀도 숫자가 꼼짝 않던 이유가 이것이었다 —
     * 막힐 때마다 회전이 통째로 풀리고 있었다.
     *
     * 한 단계를 사이에 둔다. 먼저 이틀만 미루는 이력으로 다시 찾아보고,
     * 그래도 없을 때만 전부 연다. 어제 드신 것이 오늘 또 오르는 일은 그래도 막힌다.
     */
    const looser = new Map<string, number>()
  ...
  for (const slot of MEAL_SLOTS) {
     ...
     * 간식 자리를 억지로 채우지 않는다.
     *
     * 이 단계는 '빈 끼니에는 뭐라도 놓는다' 는 뜻이다. 세 끼는 비면 결식이지만
     * 간식은 비어도 결식이 아니다. 자리를 둘로 나눈 뒤 이 단계가 둘을 다 채우는 바람에,
     * 목표 1,241 kcal 인 분의 하루가 1,492 까지 올라갔다.
     * 그렇다고 둘 다 건너뛰면 간식이 아예 없는 하루가 된다 — 실제로 그렇게 만들어 보았다.
    ...
    if (thin && snackEntries(meals).length >= cap) continue
    const cur = running()
    const room = {
      kcal: target.kcal[1] - (cur.kcal ?? 0),
      na: naLimit - (cur.na ?? 0)
   ...
   */
  {
      ...
      for (const entry of meals[slot]) {
        ...
        const swap = seasonal.find(
          (c) =>
             ...
             * 네 계절이 모두 같아진다.
             */
            (c.food.group === entry.food.group || isSnack(slot)) &&
     ...
     */
    {
       ...
       * 채우는 차례.
       *
       * 주식이 먼저다 — 반찬은 곁들임이라 밥이 없으면 아무리 놓아도 상이 서지 않는다.
       * 그다음이 국, 그다음이 주찬·부찬이다. 밥에 국 한 그릇이 가장 흔한 모습이다.
       */
      const order: MealRole[] = ['staple', 'soup', 'main', 'side']
      for (const slot of ['아침', '점심', '저녁'] as MealSlot[]) {
        const here = meals[slot]
         ...
         */
        if (!roles.includes('staple')) {
          const spare = here
            .filter((e) => mealRole(e.food) !== 'soup' && mealRole(e.food) !== 'supp')
            /* 상을 세우는 반찬이 하나뿐이면 그것은 못 내린다 */
            .filter((e) => !isAnchorDish(e.food) || here.filter((x) => isAnchorDish(x.food)).length > 1)
            .sort((a, b) => (foodContribution(a.food, a.servings).kcal ?? 0) -
                            (foodContribution(b.food, b.servings).kcal ?? 0))
          for (const drop of [undefined, ...spare]) {
            const freed = drop ? foodContribution(drop.food, drop.servings) : {}
            const rice = candidates
              .filter((c) => !used.has(c.food.id))
              .filter((c) => mealRole(c.food) === 'staple')
              .filter((c) => slotsFor(c.food).includes(slot))
              .filter((c) => !stapleClash(meals, slot, c.food, drop))
              .filter((c) => c.kcal <= (freed.kcal ?? 0) + Math.max(0, room.kcal))
              .sort((a, b) =>
                (Number(tooSoon(a.food, recent)) - Number(tooSoon(b.food, recent))) ||
            ...
            used.add(rice.food.id)
            foodTotals = addTotals(foodTotals, foodContribution(rice.food, 1))
            here.push({
              food: rice.food, servings: 1, origin: 'added',
              contribution: drop
                ? '밥을 놓으려고 가벼운 곁들임 하나를 내렸습니다'
                : '반찬만으로는 한 끼가 되지 않아 밥을 놓았습니다',
              ruleTitle: '반찬은 밥에 곁들이는 것이라, 밥이 있어야 한 상이 됩니다.',
              refIds: [], seasonal: rice.seasonal
            })
            break
          }
          if (here.some((e) => mealRole(e.food) === 'staple')) continue
        }

        let picked: Cand | undefined
        for (const role of order) {
          /* 이미 있는 자리는 건너뛴다 — 밥에 밥, 국에 국을 얹지 않는다 */
          if (role === 'staple' && roles.includes('staple')) continue
          if (role === 'soup' && roles.includes('soup')) continue
          const fits = candidates
             ...
             */
            /* 주식 자리에서는 반찬 여부를 따지지 않는다 — 밥은 반찬이 아니다 */
            .filter((c) => (role === 'staple' ? true : isAnchorDish(c.food)))
            .filter((c) => slotsFor(c.food).includes(slot))
            /*
             * 여기에도 겹침 검사를 걸어야 한다.
             *
             * 이 걸러내기에 sideClash 가 빠져 있었다. 짜임새를 맞추는 자리가
             * 드물게 돌 때는 드러나지 않다가, 상차림 기준을 조이자
             * 한 끼에 국이 둘 오르는 일이 5.6% 로 튀어나왔다 —
             * 미역국 옆에 콩나물국. 국은 한 그릇이면 족하다.
             */
            .filter((c) => !stapleClash(meals, slot, c.food) && !sideClash(meals, slot, c.food))
            .filter((c) => {
             ...
             */
            /*
             * 곁들임은 가볍지만 밥 한 공기는 300 kcal 쯤 된다.
             * 같은 잣대로 재면 주식은 영영 못 올라가고, 반찬만 쌓인 상이 남는다.
             */
            .filter((c) => c.kcal <= Math.min(role === 'staple' ? 340 : 160, Math.max(0, room.kcal)) &&
   ...
   */
  {
    ...
    for (let pass = 0; graze && pass < 3 && snackKcal(meals) < snackShare(quota) * 0.6; pass++) {
      if (snackEntries(meals).length >= cap) break
      ...
      const movable = meals[from].filter((e) => {
        if (!SNACK_SLOTS.some((sl) => slotsFor(e.food).includes(sl))) return false
        if (snackEntries(meals).some((x) => x.food.id === e.food.id)) return false
        const capG = SLOT_GROUP_CAP[e.food.group] ?? SLOT_GROUP_CAP_DEFAULT
        if (snackEntries(meals).filter((x) => x.food.group === e.food.group).length >= capG) return false
      ...
      const want = snackShare(quota) - snackKcal(meals)
      ...
      /*
       * 두 간식 자리 가운데 아직 가벼운 쪽에 놓는다.
       *
       * 자리가 하나뿐일 때는 고민할 것이 없었는데, 오전·오후로 나뉘고 나서는
       * 한쪽에만 쌓이면 나눠 드시라고 해 놓고 실제로는 한 번에 몰아 드리는 꼴이 된다.
       * 그 음식이 갈 수 있는 자리 중에서 지금 덜 찬 쪽을 고른다.
       */
      const to = SNACK_SLOTS
        .filter((sl) => slotsFor(best.food).includes(sl))
        .sort((a, b) => (load(a) - quota[a]) - (load(b) - quota[b]))[0] ?? SNACK_SLOTS[0]
      meals[to].push(best)
    }
  }
   ...
   */
  {
    ...
    for (let pass = 0; pass < 6; pass++) {
      ...
      for (const over of overs) {
        ...
        // 끼니를 비우면서까지 옮기지 않는다
        if (!isSnack(over) && meals[over].length <= 1) continue
  ...
  const slotTotals: Record<MealSlot, NutrientTotals> = { 아침: {}, 오전간식: {}, 점심: {}, 오후간식: {}, 저녁: {} }
  for (const slot of MEAL_SLOTS) {
    /*
     * 간식 자리를 억지로 채우지 않는다.
     *
   ...
   * 틀리지는 않지만 밥상 같지도 않다.
   *
   * 여유가 있을 때만 한 가지를 더 놓는다. 상을 차리자고 하루 열량을 넘기면
   * 앞뒤가 바뀐 일이 되므로, 남은 여유 안에서만 고른다.
   */
  {
    const naLimit2 = naLimit
    for (const slot of ['아침', '점심', '저녁'] as MealSlot[]) {
      const here = meals[slot]
      if (here.length === 0) continue
      const roles = here.map((e) => mealRole(e.food))
      /* 한 그릇 음식은 그것으로 완결이다 — 국수에 반찬을 붙이지 않는다 */
      if (roles.includes('onedish') || !roles.includes('staple')) continue

      /* 국은 반찬 수에 넣지 않는다 — 밥·국은 따로 세고 반찬만 센다 */
      const sides = here.filter((e) => isAnchorDish(e.food) && mealRole(e.food) !== 'soup')
      if (sides.length >= 2) continue

      const cur = running()
      const room = {
        kcal: target.kcal[1] - (cur.kcal ?? 0),
        na: naLimit2 - (cur.na ?? 0),
        /*
         * 단백질 여유도 함께 본다.
         *
         * 열량과 나트륨만 보고 반찬을 한 가지 더 놓았더니, 신장이 걸리는 분의
         * 단백질 초과가 0.79% 에서 1.50% 로 올라갔다. 반찬은 대개 단백질 식품이다 —
         * 상 모양을 갖추자고 신장에 부담을 주는 것은 순서가 뒤바뀐 일이다.
         */
        protein: target.protein[1] - (cur.protein ?? 0)
      }
      /*
       * 여유가 없으면 나눠 담는다.
       *
       * 열량 여유만 보고 반찬을 더하려니, 밥을 먼저 앉힌 뒤로는 남는 자리가 거의 없어
       * 두 가지 이상인 상이 셋 중 하나에 그쳤다. 그런데 한식의 반찬 두세 가지는
       * '더 먹는 것' 이 아니라 '나눠 먹는 것' 이다 — 접시가 늘면 한 접시가 작아진다.
       * 그래서 있던 반찬을 반으로 줄이고 그 자리에 다른 반찬을 반접시 놓는다.
       * 하루 합계는 그대로이면서 상은 한식다워진다.
       */
      const halve = room.kcal < 55 || room.protein < 3
      const victim = sides.find((e) => e.origin !== 'chosen' && e.servings === 1)
      if (halve && !victim) continue
      /*
       * 나누는 것은 하루 열량이 이미 찼을 때만 한다.
       *
       * 이 걸림돌을 빼놓았더니, 아직 목표에 못 미치는 날에도 접시를 반으로 갈랐다.
       * 새로 놓는 반찬이 대개 더 가벼워서 하루 합계가 오히려 줄었고,
       * 열량 미달이 계절·체중 검사에서 열일곱 자리로 늘었다.
       * 상 모양은 배가 부른 다음의 이야기다 — 모자란 날에는 나누지 않는다.
       */
      if (halve && (cur.kcal ?? 0) < target.kcal[0]) continue

      /*
       * 나물 한 접시까지 단백질로 막지는 않는다.
       *
       * 단백질 여유가 0 이면 김치도 못 올라간다. 그런데 상을 갖추는 반찬은
       * 대개 나물·김치·채소라 단백질이 2 g 안팎이다.
       * 다만 신장이 걸리는 분께는 그 2 g 도 함부로 더하지 않는다 — 신장이 먼저다.
       */
      if (!renalCap) room.protein = Math.max(room.protein, 3)
      if (halve && victim) {
        const back = foodContribution(victim.food, 0.5)
        victim.servings = 0.5
        foodTotals = addTotals(foodTotals, negate(back))
        room.kcal += back.kcal ?? 0
        room.na += back.na ?? 0
      ...
      if (!add) {
        ...
        if (halve && victim) {
          victim.servings = 1
          foodTotals = addTotals(foodTotals, foodContribution(victim.food, 0.5))
        }
        continue
      }

      const part = halve ? 0.5 : 1
      used.add(add.food.id)
      foodTotals = addTotals(foodTotals, foodContribution(add.food, part))
   ...
   * 밥, 국, 주찬, 반찬, 그리고 후식 — 실제로 상에 오르는 차례를 그대로 따른다.
   */
  {
    const rank: Record<MealRole, number> = {
      onedish: 0, staple: 0, soup: 1, main: 2, side: 3, dessert: 4, supp: 5
    }
    for (const slot of Object.keys(meals) as MealSlot[]) {
      meals[slot] = meals[slot]
        .map((e, i) => ({ e, i }))
        /* 같은 자리끼리는 원래 차례를 지킨다 — 뒤섞으면 고르신 것이 밀려난다 */
        .sort((a, b) => (rank[mealRole(a.e.food)] - rank[mealRole(b.e.food)]) || (a.i - b.i))
        .map((x) => x.e)
    }
  }
...
function sideClash(meals: Record<MealSlot, MenuEntry[]>, to: MealSlot, food: Food, replacing?: MenuEntry): boolean {
   ...
   * 국은 한 끼니에 하나다.
   *
   * 미역국과 시금치된장국이 한 저녁에 나란히 올랐다. 국을 두 그릇 놓고 드시지 않는다.
   * 국을 당기는 점수를 넣은 뒤로 이 일이 생겼다 — 당기기만 하고 뚜껑을 안 덮은 셈이다.
   */
  if (mealRole(food) === 'soup' && here.some((e) => mealRole(e.food) === 'soup')) return true
   ...
   *
   * 아침에 두부(부침용), 점심에 두부부침이 올랐다. 끼니마다 따로 보아서 그렇다 —
   * 한 끼니만 보면 겹치지 않지만 하루로 보면 같은 반찬을 두 번 드시는 것이다.
   * 밥은 그날 여러 끼니에 오르는 것이 자연스러우므로 뺀다.
   */
  if (mealRole(food) === 'staple') return false
  for (const sl of MEAL_SLOTS) {
    if (sl === to) continue
    if (meals[sl].some((e) => e !== replacing && mealRole(e.food) !== 'staple' &&
        coreName(e.food.name) === core)) return true
  }
  return false
}

/*
 * 최근에 드신 것인가 — 짜임새를 맞출 때도 본다.
 *
 * 다만 여기서는 '거르지' 않고 '뒤로 미룬다'.
 * 걸렀더니 상을 세울 반찬을 못 찾아 밥상 미달이 0.05 % 에서 1.25 % 로 올랐다.
 * 같은 반찬이 한 번 더 오르는 것과 밥 한 공기만 놓이는 것 중에서는 앞쪽이 낫다.
 *
 * 상을 세우려고 반찬을 더하거나 갈아 끼우는 단계가 최근 기록을 전혀 보지 않았다.
 * 그 단계가 4,200일 중 1,776끼니에 손을 대는데, 매번 점수가 가장 높은 같은 반찬을
...
function moveBreakfastElsewhere(
  ...
  for (let i = 0; i < meals['아침'].length; i++) {
    ...
    for (const to of [...SNACK_SLOTS, '점심'] as MealSlot[]) {
...
/** 두 간식 자리의 몫을 합한다 — '간식 전체' 를 뜻하던 자리에서 쓴다 */
function snackShare(m: Record<MealSlot, number>): number {
  return SNACK_SLOTS.reduce((n, s) => n + m[s], 0)
}
/** 두 간식 자리에 담긴 열량 */
function snackKcal(meals: Record<MealSlot, MenuEntry[]>): number {
  return SNACK_SLOTS.reduce((n, s) =>
    n + meals[s].reduce((m, e) => m + (foodContribution(e.food, e.servings).kcal ?? 0), 0), 0)
}
...
function mealShares(patient: PatientContext): Record<MealSlot, number> {
   ...
   * 간식을 오전·오후로 나눠 적는다.
   *
   * 잦은 소량이 필요한 분은 다섯 번에 나눈다 — 한 번에 많이 못 드시기 때문이다.
   * 간식 몫은 둘을 합쳐 18 % 다. 자리 하나일 때와 같은 22 % 로 두었더니
   * 두 자리가 각자 채워지면서 38 kg 인 분의 하루가 목표를 20 % 넘겼다.
   * 자리가 늘어난 만큼 한 자리의 몫은 줄여야 한다.
   * 그렇지 않은 분께도 자리는 두되 몫을 작게 잡는다. 오후를 조금 더 두는 것은
   * 점심과 저녁 사이가 가장 길고, 그 사이에 시장하시기 때문이다.
   */
  return grazing
    ? { 아침: 0.27, 오전간식: 0.09, 점심: 0.28, 오후간식: 0.09, 저녁: 0.27 }
    : { 아침: 0.25, 오전간식: 0.03, 점심: 0.30, 오후간식: 0.07, 저녁: 0.35 }
}
...
function snackCap(patient: PatientContext): number {
  ...
  /*
   * 자리가 둘로 나뉘었으므로 한 자리에 둘까지.
   *
   * 처음에는 잦은 소량이 필요한 분께 자리마다 셋을 주었더니, 둘을 합쳐 여섯이 되어
   * 하루 열량이 목표를 크게 넘겼다. 자리는 늘었어도 하루는 그대로다.
   */
  return needsGrazing ? 2 : 2
}
...
function placeIn(
  ...
  const pool = slotsFor(food).filter((s) => {
     ...
     */
    /* 뚜껑은 자리마다 센다 — 오전과 오후를 합쳐서 보면 한쪽에만 몰릴 수 있다 */
    return meals[s].length < cap && loadOf(s) + kcalOf <= quota[s] * 1.5
  })
  ...
  const mains = pool.filter((s) => !isSnack(s))
...
function collectCandidates(
  ...
  for (const f of CURATED_FOODS) {
     ...
     * 고르신 요리 계통에 가산점.
     *
     * 여태 계통은 거르기만 했다 — 일식을 고르시면 일식이 후보에 들어올 뿐,
     * 뽑힐 이유가 따로 없었다. 한식이 압도적으로 많으니 점수 싸움에서 늘 밀렸고,
     * 고르신 계통이 실제로 상에 오르는 날이 절반을 조금 넘는 데 그쳤다.
     * 고르셨다는 것은 드시고 싶다는 뜻이므로, 그만큼은 얹어 준다.
     */
    const cui = f.cuisine
    if (cui && cui !== '한식' && cui !== '무관' && cuisines.includes(cui)) bonus += 10
...
function bestFiller(
  ...
  for (const c of all) {
     ...
     */
    /*
     * 다만 이 틈은 품목마다 열린다. 27만 일을 돌려 보니 그 탓에 조금씩 새어,
     * 신기능이 떨어진 분 131일치에서 목표 상단의 1.3 배를 넘었다
     * (55 kg 인 분께 목표 83 g 인데 117 g). 열량이 모자란 것은 단백질이 아닌 것으로 채워야 한다.
     * 그래서 품목별 틈과 별개로, 하루 총량에 넘을 수 없는 선을 하나 더 둔다.
     * 폭은 15 % 로 두었다 — 8 % 로 조여 봤더니 오히려 나빠졌다(0.02 % → 0.07 %).
     * 여기서 막으면 다른 경로가 대신 채우기 때문이라, 조이는 것이 늘 낫지는 않다.
     */
    if (renalCap && proteinCeiling > 0 && room.protein < -(proteinCeiling * 3 * 0.15)) continue
    ...
    if (ago !== undefined && ago <= blockDays(c.food) && countsAsRepeat(c.food, c.kcal)) continue
...
function pickForSlot(
  ...
  const fresh = all.filter((c) => {
    const ago = recent.get(c.food.id)
    return !(ago !== undefined && ago <= blockDays(c.food) && countsAsRepeat(c.food, c.kcal))
  })
