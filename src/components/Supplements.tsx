        ...
        scanSupplements(kw, 400)
          .then((rows) => {
            const kept = rows.filter((r) => {
              ...
              /*
               * 권하려면 두 가지를 모두 만족해야 한다.
               *
               * 하나, 주된 성분이 이분께 권장이어야 한다 — '이 제품이 무엇을 위한 것인가'.
               * 유산균 한 통에 비타민 D 가 곁들여 들었다고 해서
               * 비타민 D 를 채우러 그것을 사시라고 할 수는 없다.
               *
               * 둘, 제품 어디에도 주의·금기 원료가 없어야 한다.
               * 앞의 조건만 두었더니 방사선치료 중인 분께
               * '알티지 오메가3 골드(오메가3 + 비타민E)' 가 권장으로 나갔다 —
               * 오메가3 는 권장이 맞지만 치료 중 고용량 비타민 E 는 재발을 늘린 성분이다.
               * 한 통에 담겨 오는 이상 따로 뗄 수 없으니, 그런 제품은 권하지 않는다.
               * 권한다는 것은 "사서 드셔도 됩니다" 라는 뜻이고, 그 안에 피해야 할 것이
               * 들어 있으면 그 말이 성립하지 않는다.
               *
               * 말리는 쪽은 반대로 곁들여 든 것까지 본다.
               * 놓쳐서 생기는 손해가 한쪽은 헛걸음이고 다른 쪽은 해가 되는 일이다.
               */
              if (pick === 'good')
                return v.primaryLevel === 'prefer' && v.level !== 'caution' && v.level !== 'avoid'
        ...
        {pick !== 'none' && (
          <div className="mb-3 rounded-xl bg-stone-50 px-3 py-2.5 ring-1 ring-stone-200">
            ...
            <p className="mt-1 text-[11px] leading-relaxed text-stone-400">
              {pick === 'good'
                ? '그 원료가 제품의 주된 성분이면서, 같은 통에 주의하실 성분이 함께 들어 있지 않은 것만 골랐습니다 — 좋은 것과 피할 것이 한 통에 담겨 오면 따로 뗄 수가 없으니까요. 공개 자료에 함량이 없어 제품 이름과 표시된 기능성으로 판단했으니, 드시기 전에 성분표를 확인하고 담당 선생님과 상의하세요.'
