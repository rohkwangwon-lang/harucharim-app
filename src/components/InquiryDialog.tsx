    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative flex max-h-[88vh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl bg-white sm:rounded-3xl">
        <div className="shrink-0 border-b border-stone-100 px-5 pb-3 pt-4">
          ...
          <h3 className="text-lg font-bold text-stone-900">문의하기</h3>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {!isSupabaseConfigured ? (
            <div className="rounded-xl bg-stone-50 px-4 py-6 text-center">
              <p className="text-sm font-medium text-stone-700">문의 기능을 준비하고 있습니다</p>
            ...
            <>
              ...
              <input
                ...
                maxLength={120}
              />
              <p className="mb-4 text-[11px] leading-relaxed text-stone-400">{KIND_HINT[kind]}</p>
              ...
              {user ? (
                <div className="mb-4 flex items-center justify-between rounded-xl bg-stone-50 px-3.5 py-2.5">
                  <div className="min-w-0">
                    <p className="text-xs text-stone-500">답변받으실 계정</p>
                    <p className="truncate text-sm font-medium text-stone-800">{displayName(user)}</p>
                  </div>
                  <button className="shrink-0 text-xs text-stone-400 hover:text-stone-600" onClick={signOut}>
                    로그아웃
                  </button>
                </div>
              ) : (
                <>
                  ...
                  />
                  <div className="mb-4 rounded-xl border border-stone-200 px-3.5 py-3">
...
function MyList({ items }: { items: Inquiry[] }) {
  if (items.length === 0) {
    return <div className="rounded-xl bg-stone-50 px-4 py-8 text-center text-sm text-stone-400">남기신 문의가 없습니다.</div>
  }
  return (
