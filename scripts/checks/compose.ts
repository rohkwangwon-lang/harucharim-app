 ...
 * 마시는 것인가.
 *
 * 처음에는 이름 어디에든 '우유' 가 있으면 음료로 보았다. 그래서 '귀리죽(우유)' 이
 * 후식으로 잡혀, 죽을 먼저 놓은 상이 모두 '후식이 앞에 끼어 있음' 이 되었다(32.5 %).
 * 괄호 안은 무엇으로 만들었는지를 적은 것이지 그것이 무엇인지가 아니다 —
 ...
 */
{
  ...
  for (const [label, extra] of [
    ...
    /*
     * 한 가지에 쏠리지 않는가 — 횟수가 아니라 몫으로 본다.
     *
     * 처음에는 '스무여드레에 몇 번' 으로 세었다. 그런데 끼니를 다섯으로 나누자
     * 하루에 담기는 것이 늘어, 가짓수가 늘고 쏠림이 줄었는데도 횟수만 올라갔다
     * (유방암 42종·21% → 44종·20% 인데 가장 잦은 것은 8회에서 10회로).
     * 세는 자가 끼니 수에 딸려 다니면 나아진 것을 나빠진 것으로 읽는다.
     *
     * 그래서 '그분이 만난 반찬 전체에서 한 가지가 차지하는 몫' 으로 본다.
     * 열에 하나를 넘으면 물린다고 본다. 다만 아무리 몫이 작아도
     * 이틀에 한 번꼴로 오르는 것은 그 자체로 잦으므로 함께 막는다.
     */
    const servings = [...seen.values()].reduce((a, b) => a + b, 0)
    if (worst && worst[1] / Math.max(1, servings) > 0.12)
      bad('한 반찬에 쏠림', `${label}: ${worst[0]}가 ${worst[1]}회로 전체의 ${Math.round(worst[1] / servings * 100)}%`)
    if (worst && worst[1] > DAYS2 / 2)
      bad('같은 반찬이 너무 잦음', `${label}: ${worst[0]}가 ${DAYS2}일에 ${worst[1]}회`)
