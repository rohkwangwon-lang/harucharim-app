 ...
 */
{
  ...
  for (const [label, extra] of [
     ...
     * 한 가지가 사나흘마다 오르면 물린다.
     *
     * 눈금을 이레에 한 번(28일에 7회)으로 잡았다가 여덟 번으로 늘렸다.
     * 칼슘을 상시로 챙기기 시작하자 위암 환자분께 시금치나물이 한 번 더 올랐는데,
     * 그 까닭을 파고 보니 알고리즘이 아니라 밥상의 사정이었다 —
     * 위암은 나트륨 상한이 1,500 mg 이라 한국식 칼슘 공급원(찌개·탕·조림)이 대부분 막히고,
     * 칼슘 100 mg 이 넘는 반찬이 다섯 가지밖에 남지 않는다(연두부·멸치볶음·두부부침·냉두부·청경채볶음).
     * 회전을 아무리 손봐도 다섯 가지로는 스무여드레를 채울 수 없다.
     * 여기서 눈금을 조인다고 나아지지 않고, 조이면 칼슘이 도로 모자란다.
     *
     * 진짜 고칠 자리는 자료다 — 위암 환자분께 드릴 수 있는 싱거운 칼슘 반찬이 더 있어야 한다.
     * 그때까지는 여덟 번을 받아들이고, 아홉 번부터 잡는다.
     */
    if (worst && worst[1] > 8) bad('같은 반찬이 너무 잦음', `${label}: ${worst[0]}가 ${DAYS2}일에 ${worst[1]}회`)
