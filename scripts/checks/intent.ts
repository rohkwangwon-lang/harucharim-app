 ...
 * 눈금을 0.60 으로 올린다.
 *
 * 삶은 면(사리)을 메뉴에서 뺐을 때 47 % 까지 떨어졌다. 사리를 빼는 것은 맞지만
 * 그것들이 일식·동남아 후보의 상당수여서, 고르셔도 섞일 자리가 없어진 것이었다.
 * 점수를 올려 억지로 끼워 넣는 대신 자료를 채웠다 —
 * 일식·중식·동남아 요리 20종을 더하니 69 % 가 되었다.
 *
 * 없는 음식을 자꾸 내놓게 만드는 대신 있는 음식을 늘리는 쪽이 옳다.
 */
const cCuisine = claim('요리 계통', '고르신 계통이 실제로 섞인다', 0.6)
