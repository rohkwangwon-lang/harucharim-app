 ...
 */
{
  ...
  if (terms) {
    ...
    /*
     * 면책 조항에는 고의·중과실 단서가 반드시 붙어야 한다.
     *
     * 약관규제법 제7조 제1호는 '고의 또는 중대한 과실로 인한 법률상 책임을 배제하는 조항' 을
     * 무효로 본다. 그러니 '책임지지 않습니다' 만 써 두면 그 조항이 통째로 무효가 될 수 있고,
     * 그러면 가벼운 실수에 대한 면책까지 함께 날아간다.
     * 이 단서는 만든 쪽을 불리하게 하는 문구가 아니라 나머지를 살리는 장치다 — 지우지 말 것.
     */
    no(!/중대한 과실/.test(terms), '면책 조항에 고의·중과실 단서가 없음 — 약관규제법 제7조로 조항 전체가 무효가 될 수 있다')
    /* 다툼이 생겼을 때 갈 곳을 알려 주는가 */
    no(!/소비자상담센터|분쟁조정/.test(terms), '다툼이 생겼을 때 도움을 청할 곳이 적혀 있지 않음')
  }
