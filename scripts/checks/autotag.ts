...
/*
 * Bailey 2013(CMAJ) 원문 대조로 라임·포멜로·세비야 오렌지를 넣었다.
 * 같은 문단이 "네이블·발렌시아 같은 단맛 오렌지는 푸라노쿠마린이 없어 상호작용하지 않는다" 고 적으므로
 * 보통 오렌지는 잡지 않아야 한다 — 안 잡는 쪽도 함께 못 박는다.
 */
const GRAPE_YES = ['과ㆍ채주스_자몽 주스', '기타차_자몽 그린티 (J)', '라임 에이드', '라임주스', '포멜로']
const GRAPE_NO = [
  '더 듬뿍 오렌지 앤 자몽 케이크', '저분자몽모랑시타트체리콜라겐스틱',
  /* '프라임' 은 라임이 아니다 */
  '간편조리세트_프라임스테이크 부채살', '양파_레드프라임 생것', '닭튀김_프라임핫쵸킹 치킨',
  /* 단맛 오렌지는 상호작용하지 않는다 */
  '오렌지', '오렌지 주스', '발렌시아 오렌지'
]
