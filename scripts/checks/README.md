...
node_modules/.bin/jiti scripts/checks/login.ts    # 로그인 문 — 소셜이 없는 분의 길, 재설정 순서, 오류 문구, 날짜 문, 편지 서식
node_modules/.bin/jiti scripts/checks/expiry.ts   # 기한 목록이 키·바깥 주소·도메인을 다 덮는가, 휴면 점검 간격
